...
double (*WELLRNG44497b_d)(void);
uint64_t (*WELLRNG44497b_ul)(void);

void InitWELLRNG44497b(uint32_t *init){
   int j;
   state_i=0;
   case_i=0;
   WELLRNG44497b_d = case_d_1;
   WELLRNG44497b_ul = case_ul_1;
   for(j=0;j<R;j++)
      STATE[j]=init[j];
   for(j=0;j<R;j++)
      WELLRNG44497b_ul(); // scrambles a bit
}

void GetSeedWELLRNG44497b(uint32_t *seed){
   int j;
   seed[R] = case_i;
   for(j=0;j<R;j++)
      seed[j]=STATE[j];
...
void SetSeedWELLRNG44497b(uint32_t *seed){
   int j;
   case_i = seed[R];
   case_i %= 6;
   WELLRNG44497b_d = cases_d[case_i];
   WELLRNG44497b_ul = cases_ul[case_i];
   for(j=0;j<R;j++)
      STATE[j]=seed[j];
}
...
void case_1(void){
   ...
   state_i = R-1;
   case_i=2;
   WELLRNG44497b_d = case_d_3;
   WELLRNG44497b_ul = case_ul_3;
...
void case_3(void){
   ...
   if(state_i+M1<R) {
      case_i=3;
      WELLRNG44497b_d = case_d_4;
      WELLRNG44497b_ul = case_ul_4;
   }
   y = STATE[state_i] ^ ((STATE[state_i] << 7) & TEMPERB);
   y =              y ^ ((             y << 15) & TEMPERC);
}
...
void case_4(void){
   ...
   if (state_i+M3 < R) {
      case_i=4;
      WELLRNG44497b_d = case_d_5;
      WELLRNG44497b_ul = case_ul_5;
   }
   y = STATE[state_i] ^ ((STATE[state_i] << 7) & TEMPERB);
   y =              y ^ ((             y << 15) & TEMPERC);
}
...
void case_5(void){
   ...
   if(state_i+M2 < R) {
      case_i=5;
      WELLRNG44497b_d = case_d_6;
      WELLRNG44497b_ul = case_ul_6;
   }
   y = STATE[state_i] ^ ((STATE[state_i] << 7) & TEMPERB);
   y =              y ^ ((             y << 15) & TEMPERC);
}
...
void case_6(void){
   ...
   if(state_i == 1) {
      case_i=1;
      WELLRNG44497b_d = case_d_2;
      WELLRNG44497b_ul = case_ul_2;
   }
   y = STATE[state_i] ^ ((STATE[state_i] << 7) & TEMPERB);
   y =              y ^ ((             y << 15) & TEMPERC);
}
