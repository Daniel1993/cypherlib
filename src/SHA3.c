...
// rotate functions
static uint64_t right_rotate(uint64_t x, const uint64_t i) {
    return x >>i | x << (sizeof(uint64_t) * 8 - i);
}

static uint64_t left_rotate(uint64_t x, const uint64_t i) {
    return x << i | x >> (sizeof(uint64_t) * 8 - i);
}
...
static void sha3_iteration(const uint8_t* data, uint64_t h[]) {
...
void sha3_512(void* data, uint64_t len, char* hash) {
    ...
    uint8_t buffer[block_size]; // = { 0 }; // const --> read-only, not actually a constant
    memset(buffer, 0, block_size*sizeof(uint8_t));
