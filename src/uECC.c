...
#include "uECC_platform-specific.inc"
...
uECC_VLI_API void uECC_vli_nativeToBytes(uint8_t *bytes,
    ...
    for (i = 0; i < num_bytes; ++i) {
        ...
        unsigned b_div = b / uECC_WORD_SIZE;
        unsigned b_mod = b % uECC_WORD_SIZE;
        bytes[i] = native[b_div] >> (8 * b_mod);
    }
}
